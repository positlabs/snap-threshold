...
//@input int paletteType = 0 {"widget":"combobox", "values":[{"label":"Full Palette", "value": 0}, {"label":"2-Color Gradient", "value": 1}]}

//@ui {"widget":"group_start", "label":"2-Color Properties", "showIf":"paletteType", "showIfValue": 1}
//@input vec4 colorTop = {1.0, 1.0, 1.0, 1.0} {"widget":"color"}
//@input vec4 colorBottom = {0.0, 0.0, 0.0, 1.0} {"widget":"color"}
//@ui {"widget":"group_end"}

//@input float initialYPosition = 0.5 {"widget":"slider", "min": 0.0, "max": 1.0, "step": 0.01, "label":"Initial Position"}
...
var initialValueY = script.initialYPosition;

var minEndpointX = 0.0;
var maxEndpointX = 1.0;
...
function initColorPicker() {
    thisScreenTransform = safeGetComponent(sceneObject, "Component.ScreenTransform");
...
function initPalette() {
    ...
    paletteScreenTransform = safeGetComponent(script.paletteObject, "Component.ScreenTransform");
    ...
    paletteImage = safeGetComponent(script.paletteObject, "Component.Image");
    if (!paletteImage) {
...
function initBackground() {
    ...
    backgroundScreenTransform = safeGetComponent(script.backgroundObject, "Component.ScreenTransform");
    ...
    backgroundImage = safeGetComponent(script.backgroundObject, "Component.Image");
    if (!backgroundImage) {
        printWarning("missing an Image Component!");
        return false;
    }
...
function initButton() {
    ...
    if (!buttonScreenTransform) {
        buttonScreenTransform = safeGetComponent(script.buttonObject, "Component.ScreenTransform");
        if (!buttonScreenTransform) {
...
function initColorVisual() {
    ...
    if (!colorVisualImage) {
        colorVisualImage = safeGetComponent(script.colorVisualObject, "Component.Image");
        if (!colorVisualImage) {
...
function updateColor() {
    if (localScreenPos != null) {
        ...
        if (script.paletteType == 0) {
            ...
            var RGBAColor = HSVAtoRGBA(new vec4(hue, saturation, value, 1.0));
            
            currentColor = RGBAColor;
        } else if (script.paletteType == 1) { // 2-Color Palette
            currentColor = vec4.lerp(script.colorBottom, script.colorTop, currentValue);
        }
